...
#include <vector>
#include <cmath>
#include <numeric> // For std::accumulate if needed, though not directly used here
#include <algorithm> // For std::max, std::clamp
#include <iostream>  // For optional printing
#include <stdexcept> // For error handling

// --- Constructor ---
GameLogicCpp::GameLogicCpp(bool no_print_flag) :
    // Initialize state variables to reasonable defaults
    x(0.0), y(0.0), vx(0.0), vy(0.0), fuel(0.0),
    landed(false), crashed(false), landed_successfully(false),
    landing_pad_center_x(0.0), landing_pad_y(0.0),
    no_print(no_print_flag),
    last_action(0), // Default action is Noop
    // Internal state
    ax(0.0), ay(0.0),
    time_step(0),
    // Config defaults (will be overwritten by set_config)
    initial_x(0.0), initial_y(0.0), initial_vx(0.0), initial_vy(0.0), initial_ax(0.0), initial_ay(0.0),
    pcfg_g(0.0),
...
void GameLogicCpp::set_config(
    ...
    spad_width = gcfg_spad_width; // Store starting pad width
    lpad_width = gcfg_lpad_width; // Store landing pad width

    initial_x = gcfg_x0_vec[0]; // Store the *default* initial x
    initial_y = gcfg_x0_vec[1];
    initial_vx = gcfg_v0_vec[0];
    initial_vy = gcfg_v0_vec[1];
    initial_ax = gcfg_a0_vec[0];
    initial_ay = gcfg_a0_vec[1]; // Gravity added in reset

    // Initial pad position might need to be set here if not always dynamic
    // For now, assume update_pad_positions handles it before the first reset.
...
void GameLogicCpp::recalculate_derived_values() {
    ...
    if (cfg_width > 0 && cfg_height > 0) { // Basic check
...
void GameLogicCpp::reset() {
    ...
    // Ensure derived values like pad center and ground level are up-to-date
    // recalculate_derived_values(); // Called by set_config and update_pad_positions
}
...
void GameLogicCpp::reset(double spad_x1, double lpad_x1_new) {
    ...
    // --- Crucial Change: Override initial x based on spad_x1 ---
...
void GameLogicCpp::update_physics() {
    ...
    // vx += ax; // No horizontal acceleration in Python version
    vy += ay;
...
void GameLogicCpp::check_landing_crash() {
    ...
    if (y >= ground_level) {
        ...
        if (on_landing_pad && safe_speed) {
            ...
            vy = 0.0;
            if (!no_print) {
                std::cout << "GameLogicCpp: Successful landing!" << std::endl;
            }
        } else {
            crashed = true;
            landed_successfully = false;
            ...
            if (!no_print) {
                if (!on_landing_pad) {
                    std::cout << "GameLogicCpp: Crashed - outside landing area!" << std::endl;
                }
...
std::pair<std::vector<double>, bool> GameLogicCpp::update(int action) {
    ...
    apply_action(action);
    update_physics();
    check_landing_crash(); // This now handles ground collision and state changes
...
bool GameLogicCpp::is_done() const {
    ...
    // Python version also considered fuel <= 0, but was commented out.
    // Sticking to landed/crashed based on the active Python code.
}
...
// Optional: Method to get render info (Implementation if needed)
/*
GameLogicCpp::RenderInfo GameLogicCpp::get_render_info() const {
    return {x, y, vx, vy, fuel, last_action, landed, crashed, landed_successfully};
}
*/
